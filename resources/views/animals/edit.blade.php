...
@section('title', __('Edit Friend'))

@section('content')
<div class="form-wrapper">
    <div class="glass-card form-card">
        <div class="form-header">
            <h2>{{ __('Edit Details') }}: {{ $animal->name }}</h2>
            <p>{{ __('Update the information for this animal.') }}</p>
        </div>
        ...
        <form action="{{ route('animals.update', $animal) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    <label>{{ __('Species / Category') }}</label>
                    <input type="text" name="species" class="form-control" list="speciesList" value="{{ $animal->species }}" required>
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-control" required>
                        <option value="available" {{ $animal->status == 'available' ? 'selected' : '' }}>{{ __('Available') }}</option>
                        <option value="pending" {{ $animal->status == 'pending' ? 'selected' : '' }}>{{ __('Pending') }}</option>
                        <option value="adopted" {{ $animal->status == 'adopted' ? 'selected' : '' }}>{{ __('Adopted') }}</option>
                    </select>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="description" class="form-control" rows="4" required>{{ $animal->description }}</textarea>
            </div>
                ...
                <div class="image-upload-wrapper">
                    ...
                    <div id="imagePreview" class="image-preview">
                        @if($animal->image_path)
                            <img src="{{ asset('storage/' . $animal->image_path) }}" style="width:100%; height:100%; object-fit:cover; border-radius:12px;">
                        @else
            ...
            <div class="form-actions">
                <a href="{{ route('animals.index') }}" class="btn btn-outline">{{ __('Cancel') }}</a>
                <button type="submit" class="btn btn-primary">{{ __('Update Animal') }}</button>
            </div>
        </form>
    </div>
</div>
