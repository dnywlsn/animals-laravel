...
<head>
    ...
    <title>@yield('title') - Astana Animal Shelter</title>
...
<body class="{{ Request::is('/') ? 'snap-page' : '' }}">
    <script>
    ...
    <nav class="navbar">
        ...
        <div class="nav-right">
            <div class="nav-links">
                ...
                <a href="{{ route('animals.index') }}" class="{{ Request::is('animals*') ? 'active' : '' }}">{{ __('Animals') }}</a>
                @auth
                    <a href="{{ route('dashboard') }}" class="{{ Request::is('dashboard') ? 'active' : '' }}">{{ __('Dashboard') }}</a>
                    <form action="{{ route('logout') }}" method="POST" style="display: inline;">
                        @csrf
                        <button type="submit" class="btn btn-outline" style="padding: 0.3rem 0.6rem; font-size: 0.7rem;">{{ __('Logout') }}</button>
                    </form>
                @else
                    <a href="{{ route('login') }}" class="{{ Request::is('login') ? 'active' : '' }}">{{ __('Login') }}</a>
                @endauth
            </div>
            <button class="theme-toggle" id="themeToggle" style="background:none; border:none; cursor:pointer; font-size:1.2rem;">🌓</button>
            <div class="lang-switcher">
                <a href="{{ route('lang.switch', 'kk') }}" class="lang-link {{ app()->getLocale() == 'kk' ? 'active' : '' }}">KK</a>
    ...
    <main class="{{ Request::is('/') ? '' : 'container' }}">
        @yield('content')
    </main>
