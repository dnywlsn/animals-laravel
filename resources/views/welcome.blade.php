...
<section class="snap-section hero-section">
    ...
    <div class="hero-content">
        <span class="badge">{{ __('🐾 Astana Best Animal Shelter') }}</span>
        <h1>{{ __('Give a Second Chance to a Friend') }}</h1>
        <p>{{ __('Join the largest and most caring animal shelter in the capital. We are dedicated to rescuing, healing, and finding loving homes for thousands of animals.') }}</p>
        <div class="hero-buttons">
            <a href="{{ route('animals.index') }}" class="btn btn-primary" style="font-size: 1rem; padding: 1rem 2rem;">{{ __('Meet Our Animals') }}</a>
            <a href="{{ route('register') }}" class="btn btn-outline" style="font-size: 1rem; padding: 1rem 2rem;">{{ __('Join the Mission') }}</a>
        </div>
    </div>
</section>
...
<section id="mission" class="snap-section mission-section">
    <div class="container">
        <h2 class="section-title reveal">{{ __('Our Core Mission') }}</h2>
        <div class="mission-grid">
            <div class="mission-card glass-card reveal">
                <div class="icon">🏘️</div>
                <h3>{{ __('Safe Shelter') }}</h3>
                <p>{{ __('We provide high-quality living conditions for every resident. Our shelter in Astana is equipped with modern facilities for comfort and safety.') }}</p>
            </div>
            <div class="mission-card glass-card reveal">
                <div class="icon">🩺</div>
                <h3>{{ __('Medical Care') }}</h3>
                <p>{{ __('Professional veterinary care is provided 24/7. From vaccinations to complex surgeries, we ensure every animal is healthy and strong.') }}</p>
            </div>
            <div class="mission-card glass-card reveal">
                <div class="icon">🏡</div>
                <h3>{{ __('Successful Adoption') }}</h3>
                <p>{{ __('We don\'t just find homes, we find families. Our thorough matching process ensures a lifelong bond between pets and owners.') }}</p>
            </div>
        </div>
    </div>
</section>
...
<section class="snap-section stats-section">
    <div class="container">
        <h2 class="section-title reveal"> {{ __('Our Collective Impact') }}</h2>
        <div class="achievements-grid">
            <div class="achievement-item reveal">
                <span class="number">1.5K+</span>
                <span class="label">{{ __('Rescued') }}</span>
                <span class="desc">{{ __('Abandoned lives saved from the streets.') }}</span>
            </div>
            <div class="achievement-item reveal">
                <span class="number">920+</span>
                <span class="label">{{ __('Adopted') }}</span>
                <span class="desc">{{ __('Successful matches and happy families.') }}</span>
            </div>
            <div class="achievement-item reveal">
                <span class="number">120+</span>
                <span class="label">{{ __('Partners') }}</span>
                <span class="desc">{{ __('Local businesses supporting our cause.') }}</span>
            </div>
            <div class="achievement-item highlight reveal">
                <span class="number">5K+</span>
                <span class="label">{{ __('Donations') }}</span>
                <span class="desc">{{ __('Total support from our community.') }}</span>
            </div>
        </div>
    </div>
</section>
...
<section class="snap-section faq-section">
    <div class="container">
        <h2 class="section-title reveal"> {{ __('Everything You Need to Know') }}</h2>
        <div class="faq-grid">
            <div class="faq-item glass-card reveal">
                <h4>🐕 {{ __('What is the adoption process?') }}</h4>
                <p>{{ __('Browse our catalog, choose a pet, and fill out the inquiry form. We will schedule a meeting and an interview to ensure the best fit for both you and the animal.') }}</p>
            </div>
            <div class="faq-item glass-card reveal">
                <h4>📍 {{ __('Where are you located?') }}</h4>
                <p>{{ __('We are located in the heart of Astana. You can visit us during our open hours to meet the animals and see our facilities firsthand.') }}</p>
            </div>
            <div class="faq-item glass-card reveal">
                <h4>🍖 {{ __('How can I help besides adopting?') }}</h4>
                <p>{{ __('You can donate food, medicine, or funds. We also have a great volunteer program where you can help with walking and caring for the animals.') }}</p>
            </div>
            <div class="faq-item glass-card reveal">
                <h4>💉 {{ __('Are all animals vaccinated?') }}</h4>
                <p>{{ __('Yes, every animal that leaves our shelter is fully vaccinated, dewormed, and has a medical record provided to the new owner.') }}</p>
            </div>
        </div>
    </div>
</section>
...
<section class="snap-section gallery-section">
    <div class="container">
        <h2 class="section-title reveal"> {{ __('Moments of Joy') }}</h2>
...
<section class="snap-section cta-section">
    <div class="container">
        <div class="cta-card glass-card reveal">
            <h2>{{ __('Join Our Global Community') }}</h2>
            <p>{{ __('Stay updated with our latest rescues, success stories, and upcoming events. Your support changes lives.') }}</p>
            <form class="newsletter-form" onsubmit="event.preventDefault(); alert('Subscribed!');">
                ...
                <button type="submit" class="btn btn-primary">{{ __('Get Updates →') }}</button>
            </form>
            <div class="cta-footer">
                <p>{{ __('Ready to make a difference?') }} <a href="{{ route('animals.index') }}">{{ __('🐾 Explore All Animals 🐾') }}</a></p>
            </div>
        </div>
    </div>
</section>
